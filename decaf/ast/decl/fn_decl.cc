...
void FnDecl::set_parent(Symbol_table &other) { symbol_table.set_parent(other); }

bool FnDecl::return_now(CodeGenerator *codegen,
                        Frame_allocator *frame_allocator,
                        Symbol_table *symbol_table, Expr *expr) {
...
bool FnDecl::matches_prototype(FnDecl *other) {
  if (!matches_signature(other))
    return false;
  return getName() == other->getName();
}

void FnDecl::emit(CodeGenerator *codegen, Frame_allocator *frame_allocator,
                  Symbol_table *symbol_table) {
  codegen->GenLabel(get_label());
  auto function = codegen->GenBeginFunc();
  auto body_allocator =
      new Frame_allocator(fpRelative, Frame_growth::Downwards);
  auto formals_allocator =
      new Frame_allocator(fpRelative, Frame_growth::Upwards);
  auto parent_is_class = dynamic_cast<ClassDecl *>(parent);
  if (parent_is_class)
    auto thisParam = formals_allocator->allocate(4, strdup("this"));
...
const char *FnDecl::get_label() {
  ...
  if (parent_is_class)
    ...
    return strdup(
        Label_transformer::get_for_method(
            label_override ? label_override : parent_is_class->getName(),
            getName())
            .c_str());
